import React, { PropTypes } from 'react';
import { findDOMNode } from 'react-dom';
import { TextField, FlatButton, RadioButtonGroup, RadioButton } from 'material-ui';
import style from './style.scss';

class PBBarChartEditor extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      mode: 'local',
      foo: 'bar',
    };
  }

  getConfig() {
    const dataUrl = this.refs.dataUrl.getValue();
    return { dataUrl };
  }

  handlerSubmit() {
    this.props.on(this.getConfig());
  }

  render() {
    const { config, onSave, onCancel } = this.props;
    return (
      <div className={style.root}>
        <div className={style.left}>
          <RadioButtonGroup name="mode" defaultSelected="local" className={style.rgb}
                            onChange={(event, selected) => this.setState({ mode: selected })}>
            <RadioButton
              value="local"
              label="本地模式" />
            <RadioButton
              value="remote"
              label="远程模式" />
          </RadioButtonGroup>
          <TextField
            hintText="远程服务URL" ref="dataUrl"
            defaultValue={this.props.config.dataUrl}
            floatingLabelText="远程服务URL" />
          <TextField ref="text"
                     multiLine={true}
                     fullWidth={true}
                     rows={4}
                     rowsMax={8}
                     hintText="请输入正文"
                     defaultValue={config.text}
                     floatingLabelText="正文(按Enter键换行)"/>
        </div>
        <div className={style.right}>
          <TextField
            hintText="远程服务URL" ref="dataUrl"
            defaultValue={this.props.config.dataUrl}
            floatingLabelText="颜色" />
        </div>
        <div style={{
          position: 'absolute',
          right: '8px',
          bottom: '8px',
        }}>
          <FlatButton
            label="取消"
            secondary={true}
            onTouchTap={() => onCancel()}
          />
          <FlatButton
            label="提交"
            primary={true}
            keyboardFocused={true}
            onTouchTap={() => onSave(this.getConfig())} />
        </div>
      </div>
    );
  }
}

PBBarChartEditor.propTypes = {
  config: PropTypes.object.isRequired,
  onCancel: PropTypes.func.isRequired,
  onSave: PropTypes.func.isRequired,
};
PBBarChartEditor.defaultProps = {
};

export default PBBarChartEditor;
